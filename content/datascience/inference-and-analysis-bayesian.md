...
Each value of `A` or `B` represents a distinct parallel universe. We call `A` and `B` Universe Vector `A` and `B`.
    ...
    - $P(A|B)$
        ...
        - Normalize Joint Universe Matrix $P(A,B)$ with Universe Vector $P(B)$ (By row), we get the conditional Universe Matrix : $P(A|B)=\frac{P(A,B)}{P(B)}$
        >$\sum\limits_iP(a_i|b_j)=1$ 
        
        ![conditional matrix](../imgs/conditionalmatrix.jpg)
...
4. Belief Propagation
    ![joint matrix](../imgs/beliefpropagation.jpg)
    ...
    - . Two Universe Matrix:
        - `B` maintains $P(B|A)$ 
        - `C` maintains $P(C|B)$ 
        These kind of Universe Matrix are most common and easy access information.
...
4.1 when $C$ is $e^-$ 
   ...
   - Want: $\lambda(A) = P(e^-|B)$
   Our knowledge of `Universe Vector` $\lambda(B)$ can be passed
   - $P($
   TODO : Check this 
...
4.2 when $A$ is $e^+$
   - Known: $\pi(B) = P(B|e^+)$
   - Want: $\pi(C) = P(C|e^+)$
   Our predict support of `Universe Vector` $\pi(B)$ pass to the support of $C$ through `Universe Matrix` $P(C|B)$
        - $ P(B|e^+)P(C|B) \propto P(C|B,e^+)$
        - sum over `B`: $\sum \limits_jP(C|b_j,e^+) = P(C|e^+)$
   - we get $P(C|e^+)$
   - In short: $\pi(B)^T \cdot P(C|B)$

    Belief matrix
        intuitive 
        numerical 
## .Detail Recap 
Upper case letters always means a distinct dimension (Rank-1 tensor)
The number of Upper case letters is the number of Ranks this entire expression is. 

The upper case letter response for the propagation of the information.

Matrix: TWO Upper case letters 
- $P(A,B)$ 
- $P(A|B)$

Vector: One upper case letters 
- $P(A)$ or $P(B)$
- $P(A|b_i)$ or $P(a_i|B)$ 
...
. propogation rules of different branch type
. training 
